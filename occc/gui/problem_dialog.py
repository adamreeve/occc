...
class ProblemDialog(QDialog):
    """Dialog for creating a new problem simulation"""
    ...
    def __init__(self, parent=None):
        ...
        for problem_class in problem_types.keys():
            self.classCombo.addItem(problem_class,
                    userData = problem_types[problem_class])
    ...
    def updateTypes(self):
        ...
        for problem_type in self.classCombo.itemData(
                self.classCombo.currentIndex()).keys():
            self.typeCombo.addItem(problem_type,
                    userData = self.classCombo.itemData(
                    self.classCombo.currentIndex())[problem_type])

    def updateSubtypes(self):
        self.subtypeCombo.clear()
        if self.typeCombo.count() > 0:
            for problem_subtype in self.typeCombo.itemData(
                    self.typeCombo.currentIndex()).keys():
                self.subtypeCombo.addItem(problem_subtype,
                        userData = self.typeCombo.itemData(
                        self.typeCombo.currentIndex())[problem_subtype])
